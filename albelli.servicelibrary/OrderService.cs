...
namespace albelli.servicelibrary
{
    ...
    public class OrderService : IOrderService
    {
        ...
        public datacontract.IOrder Create(datacontract.IOrder order)
        {
            try
            {
                ...
                if (this.Validate(order))
                {
                    ...
                    //Note: Potentially this could be cached so we can avoid DB calls to get the product details everytime we add a new order.
        ...
        private bool Validate(datacontract.IOrder order)
        {
            ...
            //if (order.OrderId <= 0) // My thoughts are this should not be done since for a order cretaion client does not need to send a Id and the ID should really be generated by the server
            //    throw new ArgumentNullException("Invalid Order id");
