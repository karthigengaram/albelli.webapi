...
namespace albelli.test
{
    ...
    public class OrderTest
    {
        ...
        /// <summary>
        /// This test is ensure all products are ordered without mug(with 5 photoBook,4 canvas, 7 calender and 8 greeting card).
        /// </summary>
        [Test]
        public void TestOrderWidthForBin_AllProductsExceptMug()
        {
            //Arrange Fake DAL
            IOrderDAL ordersDAL;
            IOrder order;
            ...
            // Arranges the Order that needs to be passed to the controller.
            // with 5 photoBook,4 canvas, 7 calender and 8 greeting card.
            order = ArrangeOrderswithAllProductsOtherThanMug();
            A.CallTo(() => ordersDAL.AddNewOrder(order)).Returns(order);
            ...
            //Assert
            Assert.AreEqual(result.RequiredBinWidth, 266.6M);//(5*19)+(4*16)+(7*10)+(8*4.7))=164
        ...
        /// <summary>
        /// Arranges orders with 5 photoBook,4 canvas, 7 calender and 8 greeting card.
        /// </summary>
        /// <param name="mugCount">
        /// The number of mug count to pass.
        /// </param>
        ...
        /// The arranged order object.
        /// </returns>
        private IOrder ArrangeOrderswithAllProductsOtherThanMug()
        {
            datacontract.IOrder order = new datacontract.Order();
            datacontract.IOrderDetails orderDetails = new datacontract.OrderDetails();
            ...
            order.CreatedDateTime = System.DateTime.Now;
            order.OrderDetails = new List<OrderDetails>();
            order.OrderDetails.Add(ArrangeOrderDetails(ProductNames.photoBook, 5));
            order.OrderDetails.Add(ArrangeOrderDetails(ProductNames.canvas, 4));
            order.OrderDetails.Add(ArrangeOrderDetails(ProductNames.calendar, 7));
            order.OrderDetails.Add(ArrangeOrderDetails(ProductNames.greetingcards, 8));
            return order;
        }
        ...
        /// The arranged order object.
        /// </returns>
        private OrderDetails ArrangeOrderDetails(datacontract.ProductNames productNames, short quantity)
        {
            OrderDetails orderDetails = new datacontract.OrderDetails();
